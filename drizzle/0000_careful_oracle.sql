...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "NoBazir_post" ADD CONSTRAINT "NoBazir_post_created_by_NoBazir_user_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."NoBazir_user"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "created_by_idx" ON "NoBazir_post" USING btree ("created_by");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "name_idx" ON "NoBazir_post" USING btree ("name");--> statement-breakpoint
